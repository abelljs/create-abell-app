...
async function main(projectName, options) {
  ...
  console.log(
    `${colors.cyan('Directory:')} ${setProjectName}\n${colors.cyan(
      'Template:'
  ...
  if (projectName === '.') {
    ...
    if (!files.length) {
      await createProject(setProjectName, template, installer);
    } else {
  ...
  if (
    projectName !== '.' &&
    fs.existsSync(path.join(process.cwd(), setProjectName))
  ) {
  ...
  try {
    const packageName = path.basename(setProjectName);
  ...
  console.log(`${colors.green('>')} Successfully Created ${setProjectName} 🚀`);
  projectName !== '.'
    ? console.log(
      // eslint-disable-next-line max-len
        `\n${colors.green('>>')} \`cd ${setProjectName}\` and \`${
          // eslint-disable-next-line max-len
