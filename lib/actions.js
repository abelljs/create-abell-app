...
async function main(projectName, options) {
  ...
  let installerCommand = [/^win/.test(process.platform) ? 'npm.cmd' : 'npm', ['install']];
  if (installer === 'yarn') {
  ...
  try {
    ...
    console.log('> Could not add custom name to package.json. Not a big issue. Everything will still work.')
  }

  // Finish log
  console.log(`\n${colors.green('>')} Finished Installing Dependencies 🤠`);
  console.log(`${colors.green('>')} Successfully Created ${projectName} 🚀`);
  console.log(`\n${colors.green('>>')} \`cd ${projectName}\` and \`${installer === 'yarn' ? 'yarn' : 'npm run'} dev\` to run dev-server ✨\n\n`);
}
