 ...
 * @returns {Promise} 
 */
 function updatePackageJson(packagePath, appName) {
 ...
 * @description Creates a list of json files to be modified and invokes the updateFN parllely 
 * @returns {Promise} 
 */
async function setCorrectAppName(appPath) {
  try {
    const currentDirectory = process.cwd();
    const appName = appPath.split(path.sep).pop();
    const packageFilesRegex = new RegExp(/^(package|yarn|package-lock).json$/);
    const packageFilesList = fs
    .readdirSync(appPath)
    .filter(packageFilesRegex.test.bind(packageFilesRegex))
    .map(file => path.join(currentDirectory, appPath, file));

    const modifyPackagePromiseList = packageFilesList.map(pFile => updatePackageJson(pFile, appName));
    await Promise.all(modifyPackagePromiseList);
  } catch(err) {
    throw err;
  }
}
